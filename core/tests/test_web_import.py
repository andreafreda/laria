...
async def test_import_is_idempotent(client):
    await client.post("/api/finance/import", data=_upload("checking", _POSTEPAY_CSV))
    resp = await client.post("/api/finance/import", data=_upload("checking", _POSTEPAY_CSV))
    body = await resp.json()
...
async def test_import_missing_file_is_400(client):
    ...
    resp = await client.post("/api/finance/import", data=form)
    assert resp.status == 400
